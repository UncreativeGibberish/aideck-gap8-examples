...
void camera_task(void *parameters)
{
  ...
  while (1)
  {
    if (wifiClientConnected == 1)
    {
      ...
      evBits = xEventGroupWaitBits(evGroup, CAPTURE_DONE_BIT, pdTRUE, pdFALSE, (TickType_t)(150/portTICK_PERIOD_MS)); //150ms timeout
      pi_camera_control(&camera, PI_CAMERA_CMD_STOP, 0);
	  //Camera capture didn't complete in 150ms, retry until it does
      while((evBits & CAPTURE_DONE_BIT) != CAPTURE_DONE_BIT)
      {
        cpxPrintToConsole(LOG_TO_CRTP, "Failed camera acquisition\n");
        pi_camera_control(&camera, PI_CAMERA_CMD_START, 0);
        evBits = xEventGroupWaitBits(evGroup, CAPTURE_DONE_BIT, pdTRUE, pdFALSE, (TickType_t)(150/portTICK_PERIOD_MS));
        pi_camera_control(&camera, PI_CAMERA_CMD_STOP, 0);
      }
